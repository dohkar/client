...
export default function RegisterPage() {
  ...
  const handlePhoneChange = (inputValue: string) => {
    ...
    if (isPhoneInput) {
      setPhone(formatPhoneInput(inputValue));
    } else {
      setPhone(inputValue);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { contact: normalized } = normalizeContact(phone);
      await register({ phone: normalized, password });
  ...
  const handleOAuth = (provider: "google" | "yandex" | "vk") => {
    const url = authService.getOAuthUrl(provider);
    window.location.href = url;
  };

  const oauthBtnClass =
    "w-full h-10 sm:h-11 rounded-xl flex items-center justify-center gap-2 border-input hover:bg-muted/70 transition-colors";
          ...
          <div className='grid grid-cols-3 gap-2'>
            ...
            >
              <Image
                ...
                className='shrink-0'
              />
              <span className='hidden sm:inline text-sm font-medium'>Яндекс</span>
            </Button>
            <Button
              type='button'
              variant='outline'
            ...
            >
              ...
              <span className='hidden sm:inline text-sm font-medium'>VK</span>
            </Button>
          </div>
            ...
            <div className='space-y-1.5 sm:space-y-2'>
              ...
              <div className='relative'>
                ...
                <Button
                  ...
                  variant='clear'
                  size='icon'
                  className='absolute right-1 top-1/2 -translate-y-1/2 h-10 w-10 rounded-xl text-muted-foreground hover:text-foreground min-w-0'
